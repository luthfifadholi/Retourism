...
if (!isset($_POST['submit'])) {
	...
	SELECT DISTINCT ?nama ?lokasi ?htm ?deskripsi WHERE {
	  ?x1 vocab:memiliki_cuaca_id_cuaca $cuaca;
	      vocab:memiliki_cuaca_id_destinasi ?y1.
	  ?x2 vocab:memiliki_tema_id_tema ?tema. FILTER ($new_tema)
	  ?x2 vocab:memiliki_tema_id_destinasi ?y2.
	  ?destinasi vocab:destinasi_id ?faktor. FILTER (?faktor = ?y1 && ?faktor = ?y2)
